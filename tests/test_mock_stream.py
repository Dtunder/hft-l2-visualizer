...
def test_generate_mock_book() -> None:
...
def test_main_loop() -> None:
    ...
    with patch("time.sleep", side_effect=side_effect), patch(
        "builtins.print"
    ) as mock_print, patch("sys.stdout") as mock_stdout:
...
def test_main_loop_exceptions(caplog: pytest.LogCaptureFixture) -> None:
    import logging
    ...
    with patch("time.sleep", side_effect=sleep_side_effect), patch(
        "mock_stream.generate_mock_book",
        side_effect=[
        ...
        assert any(
            "Error generating or writing mock data" in record.message
            for record in caplog.records
        )
        assert any(
            "Unexpected error in mock stream" in record.message
            for record in caplog.records
        )
...
def test_main_keyboard_interrupt(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with patch("time.sleep", side_effect=KeyboardInterrupt()), patch(
